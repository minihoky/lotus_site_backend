...
const listQuerySchema = z.object({
    ...
    condominium: z.string().trim().min(1).optional(),
...
function toDescriptionArray(description) {
...
function asFiles(value) {
    ...
    if (Array.isArray(value)) {
        return value.filter((item) => item instanceof File && item.size > 0);
    }
    return [];
}
...
async function parsePropertyMultipart(body, options) {
    ...
    if (location.length < 3)
        fieldErrors.location = ["Localização é obrigatória"];
    if (descriptionRaw.length < 10)
        fieldErrors.description = ["Descrição muito curta"];
    if (!Number.isFinite(beds) || beds < 0)
        fieldErrors.beds = ["Número de quartos inválido"];
    if (!Number.isFinite(baths) || baths < 0)
        fieldErrors.baths = ["Número de banheiros inválido"];
    if (!Number.isFinite(parking) || parking < 0)
        fieldErrors.parking = ["Número de vagas inválido"];
    if (!Number.isFinite(area) || area < 1)
        fieldErrors.area = ["Área inválida"];
    const priceValue = parseBrazilianPrice(priceRaw);
    if (priceValue < 1)
        fieldErrors.price = ["Preço inválido"];
...
propertiesRouter.put("/:slug", async (c) => {
    ...
    try {
        ...
        const hasCodeField = body.code !== undefined;
