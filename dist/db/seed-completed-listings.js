...
function amenityFeature(id, parking) {
...
function featuresForListing(listingIndex, parking) {
    const targetCount = (listingIndex % 10) + 1;
    const amenityIds = AMENITY_CATALOG.map((entry) => entry.id).filter((id) => id !== "parking_space" || parking > 0);
